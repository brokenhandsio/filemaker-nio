...
public extension FileMakerNIO {
    func createRecord<T>(layout: String, data: T) -> EventLoopFuture<CreateRecordResponse> where T: Encodable {
        ...
        let url = "https://\(self.configuration.hostname)\(self.configuration.queryPath)\(self.configuration.databaseName)/layouts/\(layout)/records"
    ...
    func editRecord<T>(_ id: Int, layout: String, data: T) -> EventLoopFuture<EditRecordResponse> where T: Encodable {
        let url = "https://\(self.configuration.hostname)\(self.configuration.queryPath)\(self.configuration.databaseName)/layouts/\(layout)/records/\(id)"
    ...
    func getRecords<T>(layout: String, decodeTo type: T, offset: Int?, limit: Int?) -> EventLoopFuture<[T]> where T: Codable {
        var url  = "https://\(self.configuration.hostname)\(self.configuration.queryPath)\(self.configuration.databaseName)/layouts/\(layout)/records"
    ...
    func findRecords<T, R>(layout: String, payload:T, decodeTo type: R) -> EventLoopFuture<[R]> where T: Codable, R: Codable {
        let url = "https://\(self.configuration.hostname)\(self.configuration.queryPath)\(self.configuration.databaseName)/layouts/\(layout)/_find"
